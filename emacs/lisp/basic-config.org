...
keep its backup files to itself in a directory.
...
there, but the thing is: Since I regenerate my ~init.el~ each time I tangle this file we need to set a custom file for the operation.
...
(use-package buffer-terminator
  :straight t
  :config
  (add-to-list 'buffer-terminator-rules-alist '(kill-buffer-name . ("*Calendar*")))
