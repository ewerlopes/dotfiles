...
of indentation. The minor-mode that enables that is
...
It also loves to litter its ~init.el~ with custom variables here and there, but the thing is: Since I regenerate my ~init.el~ each time I tangle this file we need to set a custom file for the operation.
...
Emacs needs to know its master!
...
*** Frame Behavior
Well, begin with a maximized screen.
