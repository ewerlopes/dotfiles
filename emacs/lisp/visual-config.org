...
The package ~all-the-icons~ allows us to use a wide variety of icons emacs for various purposes, wherever we want, and /THAT/ is *GREAT*! I'll (ab)use this feature in my config, be warned! *NOTE*: The first time a configuration with ~all-the-icons~ loads on a machine, the needed fonts might not be available, so you'll need to install them with the command ~M-x all-the-icons-install-fonts~.
